...
<img style="display: block; margin-left: auto;margin-right: auto;width: 70%;"  alt="Logo" src="{{ URL::asset('assets/media/quotation-header-footer-images/DIPL_letter_head.png') }}" />
        ...
        <table id='maintable'
            ...
            ?>
                <td
                    style="border: 1px solid black;border-collapse: collapse;padding: 5px;vertical-align:top;width:<?php echo ${'td_width_' . $column_id}; ?>%">
...
{
?>
        <div style="padding-bottom:0px;">{{ $term_count }}) Cancellation Of Order Charge :
            {{ $cancellation_of_order_charge }}</div>
        <?php
$term_count++;
}
?>
...
{
?>
        <div style="padding-bottom:0px;">{{ $term_count }}) Validity Of Quotation : {{ $validity_of_quotation }}
        </div>
        <?php
$term_count++;
}
