...
@section('content')
@php
    $create = \Helper::getPermission('product-size-m-create') ? 1 : 0;
    $edit = \Helper::getPermission('product-size-m-edit') ? 1 : 0;
    $delete = \Helper::getPermission('product-size-m-delete') ? 1 : 0;
@endphp
    ...
    <div class="toolbar" id="kt_toolbar">
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
                <!--begin::Title-->
                <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Product Size Master</h1>
                <!--end::Title-->
                <!--begin::Separator-->
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                <!--end::Menu-->
                    ...
                    @if ($create == 1)
                        <a href="{{route('product-size-master-create') }}" class="btn btn-sm btn-primary">Add Product Size</a>
                    @endif
                ...
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($productSizeMaster as $item)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$item->unique_id ?? 'N/A'}}</td>
                                    <td>{{$item->getProductCategory->product_category ?? 'N/A'}}</td>
                                    <td>{{$item->getProductName->product_type ?? 'N/A'}}</td>
                                    <td>{{$item->product_size ?? 'N/A'}}</td>
                                    <td>{{$item->item_code ?? 'N/A'}}</td>
                                    <td>
                                        @if ($edit == 1)
                                            <a href="{{route('product-size-master-edit',['id' => $item->id])}}" title="Edit" class="menu-link flex-stack px-3" style="font-weight:normal !important;"><i class="fa fa-edit" style="font-weight:normal !important;"></i></a>
                                        @endif
                                        @if ($delete == 1)
...
<script>
    $( document ).ready(function() {
        console.log("Prodcut-size-master-index File!" );
        $('.delete').click(function(e){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{route('product-size-master-destroy')}}",
                    type: "GET",
                    data: {'id':id},
