...
@section('content')
@php
    $create = \Helper::getPermission('description-create') ? 1 : 0;
    $edit = \Helper::getPermission('description-edit') ? 1 : 0;
    $delete = \Helper::getPermission('description-delete') ? 1 : 0;
@endphp
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="modal fade" tabindex="-1" id="kt_modal_2">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">Add Sub Description1</h3>
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($indexData as $item)
                                <tr>
                                    ...
                                    <td>
                                        @if ($edit == 1)
                                            <a href="{{route('product-description-edit',['id' => $item->mid])}}" title="Edit Description" class="menu-link flex-stack px-3" style="font-weight:normal !important;" data-kt-customer-table-toolbar="base" data-bs-toggle="modal" data-bs-target="#edit_modal_{{$loop->iteration}}"><i class="fa fa-edit" style="font-weight:normal !important;"></i></a>
                                        @endif
                                                            ...
                                                            <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-1">
                                                                ...
                                                                <div class="col">
                                                                    <div class="fv-row mb-4">
                                                                        ...
                                                                        <input type="text" class="form-control form-control-solid" type="text" data-id="{{ $item->sid }}"  name="sdesc" readonly  value="{{ $item->sdesc }}" id="sdesc_{{ $item->sid }}">
                                                                    </div>
                                                                </div>
                                                                <div class="col">
                                                                    <div class="fv-row mb-4">
                                                                        ...
                                                                        <input type="text" class="form-control form-control-solid" type="text" data-id="{{ $item->mid }}"  name="mdesc"  value="{{ $item->mdesc }}" id="sdesc_{{ $item->mid }}">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                        ...
                                        @if ($delete == 1)
                                            <a data-id="{{$item->mid}}" title="Delete" style="cursor: pointer;font-weight:normal !important;" class="menu-link flex-stack px-3 delete"><i class="fa fa-trash" style="color:red;"> </i></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
...
<script>
    $( document ).ready(function() {
        ...
        $('#sub_description_id').select2({
            dropdownParent: $('#kt_modal_3')
        });
         ...
         $('.delete').click(function(e){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{route('product-description-destroyMenu')}}",
                    type: "GET",
                    data: {'id':id},
                        ...
                        if(response == 1){
                            Swal.fire(
                                ...
                                'success'
                                    ...
                                    window.location = "{{route('product-item-description')}}";
                                }
                            )
                        }else{
                            swal.fire({
        ...
        $('.save').click(function(e){
            ...
            // alert(data);
            $.ajax({
                url: "{{ route('product-description-update-menu') }}",
                type: "POST",
                headers: {
