...
@section('content')
@php
    $create = \Helper::getPermission('parent-description-m-create') ? 1 : 0;
    $edit = \Helper::getPermission('parent-description-m-edit') ? 1 : 0;
    $delete = \Helper::getPermission('parent-description-m-delete') ? 1 : 0;
@endphp
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($parentDesc as $item)
                                <tr>
                                    ...
                                    <td>
                                        @if ($edit == 1)
                                            <a href="{{route('product-description-edit',['id' => $item->id])}}" title="Edit Description" class="menu-link flex-stack px-3" style="font-weight:normal !important;" data-kt-customer-table-toolbar="base" data-bs-toggle="modal" data-bs-target="#edit_modal_{{$loop->iteration}}"><i class="fa fa-edit" style="font-weight:normal !important;"></i></a>
                                        @endif
                                                                ...
                                                                <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-1">
                                                                    <div class="col">
                                                                        <div class="fv-row mb-4">
                                                                            ...
                                                                            <input type="text" class="form-control form-control-solid" type="text" data-id="{{ $item->id }}" name="pdesc"  value="{{ $item->text }}" id="pdesc_{{ $item->id }}">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
    ...
    $( document ).ready(function() {
        ...
        $('.delete').click(function(e){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{route('product-description-destroy')}}",
                    type: "GET",
                    data: {'id':id},
        ...
        $('.save').click(function(e){
            ...
            // alert(data);
            $.ajax({
                url: "{{ route('product-description-update-parent') }}",
                type: "POST",
                headers: {
