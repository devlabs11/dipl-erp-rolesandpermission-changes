...
class processmaster extends Controller
{
    public function __construct()
    {
            ...
            $acces_management = $this->check_rights('processmaster');
    ...
    function index()
    {
        ...
        $data['module_id'] = '22.0';
        return view('processmaster.index',$data);
    }
    ...
    public function processmasterdata()
    {
        //die("here");
        $start=0;
        $length=10;
        ...
        if($search_value!="")
        {
            $search_string="where tpm.id LIKE '%$search_value%' or tpm.process_id LIKE '%$search_value%' or tpm.name LIKE '%$search_value%' or tpm.category LIKE '%$search_value%' or tpm.is_fixed_process LIKE '%$search_value%'";
            $tbldata = DB::select("select tpm.id as pid,tpm.process_id as process_id,tpm.name as name,tpm.category as category,tpm.is_fixed_process as is_fixed_process from tbl_process_master tpm $search_string limit $start,$length");

            $city_count = DB::select("select count(*) as totalcount from tbl_process_master tpm $search_string");

            //$master_count = DB::select("select count(*) as totalcount from mst_country $search_string limit $start,$length");
        }
        else
        {
            $tbldata = DB::select("select tpm.id as pid,tpm.process_id as process_id,tpm.name as name,tpm.category as category,tpm.is_fixed_process as is_fixed_process from tbl_process_master tpm limit $start,$length");
            $city_count = DB::select("select count(*) as totalcount from tbl_process_master tpm");
        }
    ...
    function submitprocessmaster(Request $request)
    {
    ...
    function editprocessmaster(Request $request)
    {
        //die("here");
        $edit_id=$request->request->get('edit_id');
